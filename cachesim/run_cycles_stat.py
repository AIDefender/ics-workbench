...
def run_once(arg):
    ...
    if tot_size_w > ass_w + 6:
        try:
            ops = np.mean([[eval(i) for i in os.popen(cmd)][0] for _ in range(10)])
...
def fmt_and_show(mod = 0):
    ...
    plt.ylabel("Cache visit cycles") 
    plt.legend(loc="upper right")
    if mod == 0:
        plt.title("Relation between cache size and cache cycles")  
    else:
        plt.title("Relation between associativity width and cache cycles")
    plt.show()
